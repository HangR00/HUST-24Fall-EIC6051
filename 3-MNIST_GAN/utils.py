...
def get_load_path_G(root, load_run=-1, checkpoint=-1):
    try:
    ...
    if checkpoint == -1:
        models = [file for file in os.listdir(load_run) if "generator_model_" in file]
        models.sort(key=lambda m: "{0:0>15}".format(m))
        # print('models',models)
        model = models[-1]
    else:
        model = "generator_model_{}.pth".format(checkpoint)

    load_path = os.path.join(load_run, model)
    return load_path

def get_load_path_D(root, load_run=-1, checkpoint=-1):
    try:
        runs = os.listdir(root)
        # TODO sort by date to handle change of month
        runs.sort()
    ...
    if checkpoint == -1:
        models = [file for file in os.listdir(load_run) if "discriminator_model_" in file]
        models.sort(key=lambda m: "{0:0>15}".format(m))
        model = models[-1]
    else:
        model = "discriminator_model_{}.pth".format(checkpoint)
...
def parse_arguments(description="NN", custom_parameters=[]):
    ...
    parser.add_argument('--lr_G', type=float, default=5e-4, help='Learning rate of G')
    parser.add_argument('--lr_D', type=float, default=5e-4, help='Learning rate of D')
...
def get_args():
    ...
    custom_parameters = [
        ...
        },
        {
            "name": "--load_run_G",
            "type": str,
            "help": "Name of the generator model to load when resume=True. If -1: will load the last run. Overrides config file if provided.",
        },
        {
            "name": "--load_run_D",
            "type": str,
            "help": "Name of the discriminator model to load when resume=True. If -1: will load the last run. Overrides config file if provided.",
        },
        {
            "name": "--checkpoint_G",
            "type": int,
            "default": 0,
            "help": "Saved generator model checkpoint number. If -1: will load the last checkpoint. Overrides config file if provided.",
        },
        {
            "name": "--checkpoint_D",
            "type": int,
            "default": 0,
            "help": "Saved discriminator model checkpoint number. If -1: will load the last checkpoint. Overrides config file if provided.",
        },
    ]
    # parse arguments
